...
symbol_positions_lines(lines::Vector{String}, symbol::Regex)::Vector{Vector{Int}} =
    map(line -> symbol_positions(line, symbol), lines)
...
function filter_overlapping(
    range::AbstractRange,
    ranges::Vector{AbstractRange}
)::Vector{AbstractRange}
    filter(other_range -> length(intersect(range, other_range)) != 0, ranges)
end

is_overlapping(range::AbstractRange, ranges::Vector{AbstractRange})::Bool =
    length(filter_overlapping(range, ranges)) == 0

function gear_ratios_pt1(lines::Vector{String})::Int
    symbol_adjacency_ranges_lines = map(
        symbol_adjacency_ranges,
        symbol_positions_lines(lines, r_symbol)
    )
    acc = 0
    for linenumber in eachindex(lines)
        for match in eachmatch(r_number, lines[linenumber])
            ...
            is_valid = is_overlapping(
                number_position_range,
                symbol_adjacency_ranges_lines[linenumber]
            )
            if linenumber > 1
                is_valid &= is_overlapping(
                    number_position_range,
                    symbol_adjacency_ranges_lines[linenumber-1]
                )
            end
...
function gear_ratios_pt2(lines::Vector{String})::Int
    ...
    symbol_adjacency_ranges_lines = map(
        symbol_adjacency_ranges,
        symbol_positions_lines(lines, r_symbol_gear)
    )
        ...
        for gear_adjacency_range in symbol_adjacency_ranges_lines[linenumber]
            ...
            for offset in -1:1
                let linenumber = linenumber + offset
                    ...
                    adjacent_number_ranges = filter_overlapping(
                        gear_adjacency_range,
                        number_ranges_lines[linenumber]
                    )
                    append!(adjacent_numbers, map(
                        range -> parse(Int, lines[linenumber][range]),
